...
创建完成后，调用[StartLoadBalancerListener](~~27597~~)接口启动监听来转发流量。
...
|XForwardedFor|String|否|on|是否开启通过X-Forwarded-For头字段获取来访者真实 IP。
...
|StickySessionType|String|否|insert|cookie的处理方式。取值：
 ...
 **说明：** 在**HealthCheck**值为**on**时才会有效。 |
|HealthCheckHttpCode|String|否|http\_2xx,http\_3xx|健康检查正常的HTTP状态码，多个状态码用逗号分隔。
