...
|---------|----|---------|-------------|-----------|
|Action|String|Yes|SetLoadBalancerTCPListenerAttribute| The name of this action.
...
|AclType|String|No|white| Optional. The access control method:
 ...
 -   **white**: Only requests from IP addresses or CIDR blocks in the selected access control list are forwarded. It applies to scenarios where the application only allows access from some specific IP addresses.
...
-   **black**: Requests from IP addresses or CIDR blocks in the selected access control list are not forwarded. It applies to scenarios where the application only denies access from some specific IP addresses.
...
|MasterSlaveServerGroup|String|No|on| Optional. Indicates whether to use an active/standby server group.
 ...
 |
|MasterSlaveServerGroupId|String|No|rsp-cige\*\*\*\*\*\*| Optional. The ID of the active/standby server group.
...
-   **tch**: A consistent hash based on the following four variables: source IP address + destination IP address + source port + destination port. The same streams are scheduled to the same backend server.
...
|VServerGroup|String|No|on| Optional. Indicates whether to use a VServer group.
 ...
 |
|VServerGroupId|String|No|rsp-cige6j5\*\*\*\*\*| Optional. The ID of the VServer group.
...
## Error codes {#section_vfj_4vz_50w .section}
