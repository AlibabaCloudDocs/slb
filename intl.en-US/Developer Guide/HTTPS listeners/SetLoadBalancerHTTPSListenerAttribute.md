...
|---------|----|---------|-------------|-----------|
|Action|String|Yes|SetLoadBalancerHTTPSListenerAttribute| The name of this action.
...
-   **1 to 5120**: The sum of the peak bandwidth values of all listeners cannot surpass the peak bandwidth of the SLB instance.
...
|Cookie|String|No|B490B5EBF6F3CD402E515D22BCDA1598| Optional. The cookie configured on the backend server.
 ...
 If the value of**StickySession** is**on** and the value of **StickySessionType** is**server**, this parameter is required.
...
|HealthyThreshold|Integer|No|4| Optional. The number of consecutive successes of health checks before a backend server is declared as healthy \(from**failure** to**success**\).
 ...
 |
|IdleTimeout|Integer|No|23| Optional. The idle connection timeout. Value range: 1 to 60. Default value: 15. Unit: seconds
...
|UnhealthyThreshold|Integer|No|4| Optional. The number of consecutive failures of health checks before a backend server is declared as unhealthy \(from**success** to**failure**\).
 ...
 |
|VServerGroup|String|No|on| Optional. Indicates whether to use an active/standby server group.
...
## Error codes {#section_u5i_faq_hsh .section}
